...
- **🎨 没入感のあるデザイン**: 再生中のアルバムアートに合わせて動的に美しく変化する背景システム。音楽の世界観に浸れる映画のような視聴体験を提供します。
- **✨ DJ INSIGHT**: AI DJ が今何を考えて選曲したか、その「意図」や「解説」をリアルタイムで表示するミニポップアップ機能を搭載。
...
- **🎯 楽曲精査 (AI Filtering)**: (実験的機能) Spotifyで検索された楽曲が、リクエスト内容に合致しているかをAIが判定し、無関係な曲を除外します。**デフォルトで有効**です。
- **🥣 自動おかわり機能 (Auto-Refill)**: 再生キューが少なくなると、DJが自動的に追加の選曲 ("おかわり") を行います。音楽が途切れることはありません。
- **📱 レスポンシブ & PWA対応**: スマホ表示にも最適化された専用レイアウト。PWAとしてデスクトップやモバイルにインストール可能です。
- **🔒 プライバシーファースト**: **Bring Your Own Key (BYOK)** アーキテクチャを採用。APIキーは利用者のブラウザ内に**ローカル保存**され、外部サーバーに送信されません。
- **🌊 シームレスなSpotify連携**: Spotify Premiumアカウントと連携し、選曲されたトラックをあなたのデバイス (スマホやPCのSpotifyアプリなど) で再生します。
